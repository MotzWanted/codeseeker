...
class PLMICDModel(PreTrainedModel):
    ...
    def __init__(
        ...
        if config.encoder is None and encoder is None:
            raise ValueError("Either an encoder configuration or an encoder has to be provided.")
        ...
        if encoder is None:
            ...
            self.config.encoder = AutoConfig.for_model(encoder_model_type, **config.encoder)
            encoder = AutoModel.from_config(self.config.encoder, add_pooling_layer=False)
            encoder.eval()
    ...
    def encode(
        ...
        chunk_input_ids = self.split_input_into_chunks(input_ids, self.config.pad_token_id)
        ...
        relevant_chunk_indices = (
            (chunk_input_ids != self.config.pad_token_id).any(dim=-1).view(-1).nonzero(as_tuple=True)[0]
        )
        relevant_chunk_input_ids = torch.index_select(chunk_input_ids.view(-1, chunk_size), 0, relevant_chunk_indices)
        ...
        out = outputs_with_padding_chunks.index_copy_(0, relevant_chunk_indices, outputs)
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits, attention = self.label_wise_attention(hidden_output, attention_mask, return_attention=return_attention)
        ...
        if targets is not None:
            ...
            loss = F.binary_cross_entropy_with_logits(logits, targets, reduction=loss_reduction)

        return {"logits": logits, "loss": loss, "attention": attention}

    def predict(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, **kwargs) -> torch.Tensor:
        logits = self(input_ids, attention_mask)["logits"]
        return self.output_activation_fn(logits)

    def split_input_into_chunks(self, input_sequence: torch.Tensor, pad_index: int) -> torch.Tensor:
        ...
        input_sequence = nn.functional.pad(
            input_sequence,
            (0, self.config.chunk_size - input_sequence.size(1) % self.config.chunk_size),
            value=pad_index,
        )
...
class LabelCrossAttention(nn.Module):
    def __init__(self, input_size: int, num_classes: int):
        ...
        self.label_representations = nn.Parameter(torch.rand(num_classes, input_size), requires_grad=True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        attention_mask = nn.functional.pad(attention_mask, (0, x.size(1) - attention_mask.size(1)), value=0)
