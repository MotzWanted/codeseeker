...
def replace_decoder_head_with_label_wise_attention(
    ...
    print(f"Replaced last layer '{last_layer_name}' with label-wise attention")
...
def has_label_wise_attention(model: torch.nn.Module) -> bool:
    ...
    """
    # Check if the model has a label-wise attention layer anywhere in the model
    for _, layer in model.named_modules():
        if isinstance(layer, LabelCrossAttention):
            return True
    return False
