...
class Arguments(pydantic.BaseModel):
    ...
    coding_system: typing.Literal["icd9", "icd10"] = "icd10"
...
def main(args: Arguments):
    ...
    if args.coding_system == "icd10":
        diag_xml_trie = XMLTrie.from_xml_file(
            _MEDICAL_CODING_SYSTEMS_DIR / "icd10cm_tabular_2025.xml",  # type: ignore
            coding_system="icd10cm",
        )
        code_lookup = {}
        for code in diag_xml_trie.all:
            if not diag_xml_trie[code].desc:
                continue
        ...
        for code in proc_xml_trie.all:
            if not proc_xml_trie[code].desc:
                continue
    ...
    for code in tqdm(codes):
        top_similarities = similarity_matrix[code].nlargest(100)
        for similar_code, _ in top_similarities.items():
            negatives[code].append((similar_code, code_lookup[similar_code]))
