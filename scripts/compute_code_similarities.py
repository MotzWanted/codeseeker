...
class Arguments(pydantic.BaseModel):
    ...
    embedding_port: int = 6546
    embedding_model: str = "BAAI/bge-multilingual-gemma2"

    coding_system: typing.Literal["icd9", "icd10cm", "icd10pcs"] = "icd10cm"
...
def main(args: Arguments):
    ...
    if args.coding_system == "icd10cm":
        ...
        for code in diag_xml_trie.all:
            code_lookup[code] = diag_xml_trie[code].desc
    elif args.coding_system == "icd10pcs":
        ...
        for code in proc_xml_trie.all:
            code_lookup[code] = proc_xml_trie[code].desc
        ...
        raise ValueError(f"Unknown coding system: {args.coding_system}")
    ...
    dataset = dataset.map(lambda x: {"embeddings": client.embed(x["descriptions"])}, batched=True, batch_size=1024)
    ...
    similarity_matrix.to_parquet(PROJECT_ROOT / f"data/{args.coding_system}_{args.embedding_model.split('/')[-1]}_similarity_matrix.parquet")
